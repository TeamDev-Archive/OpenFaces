...
public class EMailsTreeTableBean {
    ...
    private Object selectedEMail;

    public EMailsTreeTableBean() {
    ...
    public List getEMailsTreeChildren() {
        ...
        if (email == null) {
            return Arrays.asList(dataModel.keySet().toArray());
        } else {
    ...
    private List<EMail> generateEmails() {
        ...
        for (int i = 0, count = Math.max(SUBJECTS.length, SENDER_NAMES.length); i < count; i++) {
            ...
            EMail email = new EMail(sender, subject, generatedDate, EMailImportance.NORMAL, false);
            result.add(email);
        }

        int replyMessagesQuantity = rand.nextInt(20) + 40;
        ...
        for (int i = 0; i < replyMessagesQuantity; i++) {
            ...
            newEMail = new EMail(sender, subject, reDate, EMailImportance.NORMAL, false);
            result.add(newEMail);
        }
        ...
        for (int i = 0; i < result.size(); i++) {
            ...
            withImportancies.add(new EMail(email.getSender(), email.getSubject(), email.getReceivedDate(), importance, false));
        }
        result.clear();
        ...
        for (int i = 0; i < result.size(); i++) {
            ...
            withAttachments.add(new EMail(email.getSender(), email.getSubject(), email.getReceivedDate(), email.getImportance(), attachment));
        }
        result.clear();
