 ...
 */
public class TagCloudVerticalLayoutRender extends AbstractTagCloudLayoutRender {
    @Override
    protected void renderItems(FacesContext context, TagCloud cloud, List<TagCloudItem> items)
        ...
        if (!items.isEmpty()) {
            for (TagCloudItem item : items) {
                ...
                writer.startElement("br",cloud);
                writer.endElement("br");
            }
        } else {
            writer.writeText("EMPTY VERTICAL TAG CLOUD", null);
        }
    }
}
