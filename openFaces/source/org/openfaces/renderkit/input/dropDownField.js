...
O$._initDropDownField = function(dropDownId,
                                 popupTimeout,
                                 listAlignment,
  ...
  dropDown._initListStyles = function() {
    ...
    O$._initTableStyles.apply(innerTable, popupTableStructureAndStyleParams);
  ...
  dropDown._getFirstVisibleItemIndex = function() {
    return O$._dropDownGetNeighboringVisibleItem(dropDown, -1, false);
  }

  dropDown._getLastVisibleItemIndex = function() {
    return O$._dropDownGetNeighboringVisibleItem(dropDown, dropDown._items.length, true);
  }
  ...
  function completeShowingSuggestions(text, autoCompletionAllowedForThisKey) {
    if (dropDown.isOpened())
      O$._dropDown_initPopup(dropDown);
  ...
  dropDown._showCachedSuggestions = function (text, autoCompletionAllowedForThisKey) {
    ...
    O$.repaintWindowForSafari(true);
    dropDown._setHighlightedItemIndex(-1);
    dropDown._items = O$._prepareListElements(dropDown);
  ...
  dropDown._initItems = function(itemValues) {
    for (var i = 0, count = dropDown._items.length; i < count; i++) {
      ...
      item.onclick = function() {
        O$._dropDownItemClicked(dropDown, this);
      }
  ...
  //  O$._dropDownField_onLoadHandler(dropDown.id);

  // Add close on timeout behavior
  if (dropDown._popupTimeout > 0) {
    dropDown._timeOutClose = O$._dropDownCloseOnTimeout;
  ...
  dropDown.onLoadHandler = O$._dropDownField_onLoadHandler;
  ...
  // Keyboard navigation
  dropDown._handleKeyPress = O$._dropDownKeyPressHandler;
  ...
  field._handleKeyDown = O$._dropDownKeyDownHandler;
...
O$._dropDownField_onLoadHandler = function(controlId) {
...
O$._dropDownFixSize = function(dropDown) {
  var field = dropDown._field;
...
O$._dropDownKeyPressHandler = function(e) {
  var evt = O$.getEvent(e);
...
O$._dropDownGetNeighboringVisibleItem = function(dropDown, itemIndex, searchUpwards) {
...
O$._dropDownKeyDownHandler = function(e) {
  var evt = O$.getEvent(e);
  var keyCode = evt.keyCode;
  ...
  if (keyCode == 40) { // Down
    if (!dropDown.isOpened()) {
      ...
      newHighlightedItemIndex = O$._dropDownGetNeighboringVisibleItem(dropDown, highlightedItemIndex, false);
    }
  ...
  if (dropDown.isOpened()) {
    if (keyCode == 38) { // Up
      needCancelBuble = true;
      dropDown._keyNavigationStarted = true;
      newHighlightedItemIndex = O$._dropDownGetNeighboringVisibleItem(dropDown, highlightedItemIndex, true);
...
O$._dropDownItemClicked = function(dropDown, dropDownItem) {
  dropDown.closeUp()
  dropDown.focus();
...
O$._dropDownCloseOnTimeout = function(controlId) {
  var dropDown = O$(controlId);
  if (dropDown._popupCloseTimer) {
    clearTimeout(dropDown._popupCloseTimer);
  }
...
O$._prepareListElements = function(dropDown) {
...
O$._setFieldOnChange = function(controlId, func) {
  var dropDown = O$(controlId);
  dropDown.client_onchange = func;
}

O$._setDropDownOnKeyPress = function(controlId, func) {
  var dropDown = O$(controlId);
  dropDown.client_onkeypress = func;
}

O$._setDropDownOnDropDown = function(controlId, func) {
  var dropDown = O$(controlId);
  dropDown.ondropdown = func;
}

O$._setDropDownOnCloseUp = function(controlId, func) {
  var dropDown = O$(controlId);
  dropDown.oncloseup = func;
}


O$._filteredItemsLoaded = function(dropDownField, portionName, portionHTML, portionScripts) {
  ...
  if (filterCriterionStr == "null")
    ...
    O$.assert(filterCriterionStr.charAt(0) == "[", "O$._filteredItemsLoaded: Unrecognized filterCriterionStr: " + filterCriterionStr);
...
O$._acceptLoadedDropDownItems = function(dropDownFieldId, newItemParams) {
