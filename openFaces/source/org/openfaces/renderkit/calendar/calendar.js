...
O$.CURRENT_MONTH = 1;
O$.NON_CURRENT_MONTH = 2;
O$.SELECTED_DAY = 3;

O$.DAY_COUNTS = new Array(0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334);

// ================================== INTERNAL FUNCTIONS

O$._initCalendar = function(calendarId,
                            selectedDate,
                            todayDate,
                            rolloverClass,
                            focusable,
    ...
    cal[eventName] = function (evt) {
      ...
      switch (e.keyCode) {
        ...
        case 34: // page down
          ...
          break;
        case 38: // up
          O$._incSelectedDate(cal, -7);
          break;
        case 40: // down
          O$._incSelectedDate(cal, 7);
          break;
        case 37: // left
          O$._incSelectedDate(cal, -1);
          break;
        case 39: // right
          O$._incSelectedDate(cal, 1);
          break;
  ...
  if (!cal._disabled) {
    ...
    monthSelector.onmousedown = function(e) {
      O$._calendar_hideDrops(cal);
      O$._calendar_showDrop(cal, this);
      O$.breakEvent(e);
    }
    ...
    O$._calendar_initializeDrops(cal);
    O$._calendar_adjustMonthAndYearSelectorWidth(cal);
  }
  ...
  cal.getSelectedDate = function () {
    return O$._getSelectedDate(cal.id);
  }

  cal.setSelectedDate = function (date) {
    O$._setSelectedDate(cal.id, date);
  }
}

O$._getSelectedDate = function(calendarId) {
  var cal = O$(calendarId);
...
O$._setSelectedDate = function(calendarId, date) {
  var cal = O$(calendarId);
  if (cal._isDateChooser) {
    cal = cal._calendar;
  }
  ...
  if (date) {
    ...
    O$._calendar_updateDateHolder(cal, date);
    O$._updateCalendar(cal, date, true);
  } else {
    valueHolder.value = "";
  ...
  O$._notifyDateChangeListeners(cal);
}

O$._notifyDateChangeListeners = function(cal) {
  var listeners = cal._dateChange_listeners;
  if (listeners && listeners != null && listeners.length > 0) {
    for (var i = 0; i < listeners.length; i ++) {
      var listener = listeners[i];
      listener(O$._getSelectedDate(cal.id));
    }
  }
}

O$._closestDayOfMonth = function(year, month, day) {
  if (new Date(year, month, day).getDate() != day)
    return O$._closestDayOfMonth(year, month, --day);
  else
    return new Date(year, month, day);
}

O$._prevMonth = function(cal) {
  var date = O$._getSelectedDate(cal.id);
  if (!date) {
    O$._selectCurrentDate(cal);
    return;
  }
  ...
  var newDate = O$._closestDayOfMonth(year, month, day)

  if (cal._dateRanges && !O$._insideRanges(newDate, cal)) {
    return;
  }
  O$._cellClick(cal, newDate);
}

O$._insideRanges = function(date, cal) {
  var found = false;
...
O$._selectCurrentDate = function(cal) {
  O$._todayClick(cal);
}


O$._nextMonth = function(cal) {
  var date = O$._getSelectedDate(cal.id);
  if (!date) {
    O$._selectCurrentDate(cal);
    return;
...
O$._incSelectedDate = function(cal, inc) {
  var date = O$._getSelectedDate(cal.id);
  if (!date) {
    O$._selectCurrentDate(cal);
    return;
...
O$._updateCalendar = function(calendar, selectedDate, updateSelectedDateOnly) {
  ...
  if (selectedDate) {
    ...
    dates = O$._buildDatesArray(calendar, selectedDate, true);
  } else {
    if (!O$._checkMonth(calendar._currentMonth) || !calendar._currentYear) {
      calendar._currentYear = today.getFullYear();
      calendar._currentMonth = today.getMonth();
    }
    dates = O$._buildDatesArray_YMD(calendar, calendar._currentYear, calendar._currentMonth);
    monthYearChanged = true;
  }

  var tempDate = O$._getSelectedDate(calendar.id);
  ...
  for (var row = 0; row < 6; row++) {
    for (var col = 0; col < 7; col++) {
      ...
      switch (type) {
        case O$.CURRENT_MONTH:
          cell._isCurrentMonth = true;
          break;
        case O$.NON_CURRENT_MONTH:
      ...
      O$._calendar_updateDateRangesSelectedStyles(calendar, cell);

      if (!dateRanges ||
          (dateRanges && ((foundInRange && !disableIncluded) || (!foundInRange && !disableExcluded)))) {
        if (type == O$.NON_CURRENT_MONTH) {
          cell.onmouseup = function() {
            O$._cellClick(calendar, this._date);
          };
        } else if (type == O$.SELECTED_DAY) {
          cell.onmouseup = null;
        } else if (type == O$.CURRENT_MONTH) {
          cell.onmouseup = function() {
            O$._calendar_updateWithinMonth(calendar, this._date, this);
          };
        }
      } else {
        cell.onmouseup = null;
  ...
  if (calendar._showFooter) {
    O$._calendar_updateToday(calendar);
  }


  var ms = calendar._monthSelector;
  var ys = calendar._yearSelector;
  var wn = calendar._weekNumber;
...
O$._calendar_updateToday = function(calendar) {
  var date = O$._getSelectedDate(calendar.id);
  var today = new Date();
    ...
    if (calendar._applyOnclick4today && !calendar._disabled) {
      ...
      if (calendar._currentMonth == today.getMonth() && calendar._currentYear == today.getFullYear()) {
        todaySel.onmouseup = function() {
          O$._calendar_updateWithinMonth(calendar, this._todayCell._date, this._todayCell);
        };
      } else {
        todaySel.onmouseup = function() {
          O$._todayClick(calendar);
        };
      }
    } else {
...
O$._cellClick = function(cal, date) {
  O$._setSelectedDate(cal.id, O$.cloneDateTime(date));
  if (cal._onDateChange) {
    cal._onDateChange();
  }
}

O$._buildDatesArray_YMD = function(calendar, year, month, day) {
  if (!year) {
    return O$._buildDatesArray(calendar, null, false);
  }
  if (!O$._checkMonth(month)) {
    alert("Month check: " + month);
    return O$._buildDatesArray(calendar, null, false);
  }
  if (!day) {
    var date = new Date(year, month, 1);
    return O$._buildDatesArray(calendar, date, false);
  }
  return O$._buildDatesArray(calendar, new Date(year, month, day), false);
}

O$._checkMonth = function(month) {
  return month >= 0 && month < 12;
}

O$._buildDatesArray = function(calendar, date, isSelected) {
  var today = calendar._todayDate;
  if (!date)
    date = today;
  ...
  var storedSelectedDate = O$._getSelectedDate(calendar.id);
  ...
  for (var i = 0; i < 42; i ++) {
    ...
    if (y < incYear || m < incMonth) {
      ...
      pair[0] = O$.SELECTED_DAY;
    } else {
      pair[0] = O$.CURRENT_MONTH;
    }
    pair[1] = currentDay;
...
O$._calendar_compareDates = function(date1, date2) {
...
O$._decYearClick = function(cal) {
  cal._currentYear -= 1;
  O$._updateCalendar(cal);
}

O$._incYearClick = function(cal) {
  cal._currentYear += 1;
  O$._updateCalendar(cal);
}

O$._todayClick = function(cal) {
  ...
  if (cal._dateRanges) {
    ...
    if ((found && !cal._dateRanges._disableIncluded)
      ...
      O$._calendar_setTodaySettings(cal, date);
    }
  } else {
    O$._calendar_setTodaySettings(cal, date);
  }
...
O$._calendar_setTodaySettings = function(cal, date) {
  O$._setSelectedDate(cal.id, date);
...
O$._noneClick = function(calendar) {
  O$._setSelectedDate(calendar.id);
  calendar._applyOnclick4today = true;
  O$._calendar_updateToday(calendar);
...
O$._getDayOfYear = function(date) {
  var dayCounter = O$.DAY_COUNTS[date.getMonth()] + date.getDate();
  var year = date.getFullYear();
  if (O$._isLeapYear(year))
    dayCounter++;
  return dayCounter;
}

O$._isLeapYear = function(year) {
...
O$._fillDrop = function(calendar, dropTable, items, onclickHandler, boldIndex) {
  ...
  for (var i = 0, count = items.length; i < count; i ++) {
    ...
    tr.onmouseover = function() {
      O$._calendar_dropItemMouseOver(this);
      O$._operaStrictWorkaround();
    };
    tr.onmouseout = function () {
      O$._calendar_dropItemMouseOut(this);
      O$._operaStrictWorkaround();
    };
    if (applyOnclick) {
  ...
  dropTable.onmouseout = function () {
    ...
    this._timeout = setTimeout(function() {
      O$._calendar_hideDrop(dropTable)
    }, 3000);
  };
...
O$._calendar_initializeDrops = function(cal) {
  ...
  if (!dtf) {
    O$.logError("O$._calendar_initializeDrops: unsupported locale: " + cal._localeStr);
    return;
  }
  var months = dtf.getMonths();
  O$._fillDrop(cal, monthDrop, months, function() {
    O$._calendar_monthDropItemClick(cal, this);
  }, cal._currentMonth);
  ...
  O$._fillDrop(cal, yearDrop, years, function() {
    O$._calendar_yearDropItemClick(cal, this);
  }, yearsBefore);
...
O$._calendar_hideDrop = function(drop) {
  if (drop._timeout) {
...
O$._calendar_hideDrops = function(cal) {
  var monthDrop = cal._monthSelector._drop;
  var yearDrop = cal._yearSelector._drop;
  O$._calendar_hideDrop(monthDrop);
  O$._calendar_hideDrop(yearDrop);
}

O$._calendar_showDrop = function(calendar, selector) {
  var drop = selector._drop;
  O$.correctElementZIndex(drop, calendar);
  O$._calendar_initializeDrops(calendar);
  var selectorRect = O$.getElementBorderRectangle(selector, true);
  drop.showAtXY(selectorRect.x, selectorRect.getMaxY());
  O$._operaStrictWorkaround();
}

O$._calendar_dropItemMouseOver = function(dropItem) {
...
O$._calendar_dropItemMouseOut = function(dropItem) {
...
O$._calendar_monthDropItemClick = function(cal, item) {
  O$._calendar_hideDrop(item._drop);
  cal._currentMonth = item._itemNo;
  O$._updateCalendar(cal);
}

O$._calendar_yearDropItemClick = function(cal, item) {
  O$._calendar_hideDrop(item._drop);
  cal._currentYear = item._item;
  O$._updateCalendar(cal);
}

O$._operaStrictWorkaround = function() {
  if (!O$.isOpera())
    return;
...
O$._calendar_updateWithinMonth = function(calendar, date, cell) {
  var selectedCell = calendar._selectedCell;
  if (selectedCell) {
    selectedCell._isSelected = false;
    var useTempStyle = selectedCell._className !== undefined;
  ...
  O$._calendar_updateDateHolder(calendar, clonedDate);
  ...
  O$._notifyDateChangeListeners(calendar);
}

O$._calendar_updateDateRangesSelectedStyles = function(calendar, cell) {
...
O$._calendar_updateDateHolder = function(calendar, date) {
...
O$._calendar_adjustMonthAndYearSelectorWidth = function(calendar) {
  if (calendar._monthSelector._drop.clientWidth) {
    ...
    setTimeout(function() {
      O$._calendar_adjustMonthAndYearSelectorWidth(calendar)
    }, 40);
  }
}