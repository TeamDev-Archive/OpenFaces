...
public abstract class ElementInspector {
    ...
    public static void provideUtils(WebDriver driver) {
        if (utilScriptBuilder == null) {
            utilScriptBuilder = new StringBuilder();
            InputStream resourceStream = ElementInspector.class.getResourceAsStream("/funcTestsUtil-min.js");
            BufferedReader reader = new BufferedReader(new InputStreamReader(resourceStream));
            try {
                while (resourceStream.available() > 0) {
                    utilScriptBuilder.append(reader.readLine());
                }
            } catch (IOException e) {
            }
        ...
        //String script = utilScriptBuilder.toString().replace("\n", "").replace("\r", "").replace("\t", "");
        String script = utilScriptBuilder.toString();
    ...
    public String asSeleniumLocator() {
        return getSelenium().getEval("document.SeI.getXPath(" + getElementReferenceExpression() + ")");
    }
    ...
    protected String evalSeleniumInspectorExpression(String expression) {
        String fullExpression = "document.SeI." + expression;
        try {
            return getEval(fullExpression);
    ...
    public boolean elementExists() {
        String script = "!!(" + getElementReferenceExpression() + ")";
        String elementExistsStr = "false";
        try {
            elementExistsStr = getEval(script);
        } catch (Exception e) {
            return false;
        }
    ...
    public List<ElementInspector> childNodesByName(String nodeName) {
        ...
        final String subElementsRetrievalScript = "document.SeI.getQ__getChildNodesWithNames(" +
                getElementReferenceExpression() + ", ['" + nodeName + "'])";
    ...
    public Dimension size() {
        ...
        int width = (int) Math.round(Double.parseDouble(sizeArr[0]));
        int height = (int) Math.round(Double.parseDouble(sizeArr[1]));
    ...
    public Point position() {
        ...
        int x = (int) Math.round(Double.parseDouble(positionArr[0]));
        int y = (int) Math.round(Double.parseDouble(positionArr[1]));
    ...
    public Rectangle rectangle() {
        ...
        int x = (int) Math.round(Double.parseDouble(positionArr[0]));
        int y = (int) Math.round(Double.parseDouble(positionArr[1]));
        int width = (int) Math.round(Double.parseDouble(positionArr[2]));
        int height = (int) Math.round(Double.parseDouble(positionArr[3]));
    ...
    public void mouseDown() {
        getSelenium().mouseDown(getXPath());
    }
    ...
    public String adaptColorString(String color) {
        ...
        if (adaptedColor == null) {
            adaptedColor = getEval("var referenceEl = document.createElement('div'); referenceEl.style.color = '" + color + "';" +
                    "document.SeI.getQ__calculateElementStyleProperty(referenceEl, 'color');");
    ...
    public String adaptFontWeightString(String fontWeight) {
        ...
        if (adaptedFontWeight == null) {
            adaptedFontWeight = getEval("var referenceEl = document.createElement('div'); referenceEl.style.fontWeight = '" + fontWeight + "';" +
                    "document.SeI.getQ__calculateElementStyleProperty(referenceEl, 'font-weight');");
